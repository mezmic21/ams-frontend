...
export default function LecturerNotifications() {
  ...
  const t = isDark ? {
    ...
    readBg: "#0a0f1e", readBorder: "#1e2d4a",
  } : {
    ...
    readBg: "#f8fafc", readBorder: "#e2e8f0",
  };

  const getNotifStyle = (type: string) => {
    if (type === "cancellation") return { icon: "❌", color: "#ef4444", bg: isDark ? "#2d0f0f" : "#fef2f2" };
    if (type === "reschedule_pending") return { icon: "⏳", color: "#f59e0b", bg: isDark ? "#1a1200" : "#fffbeb" };
    if (type === "reschedule_approved") return { icon: "✅", color: "#10b981", bg: isDark ? "#0f2d1f" : "#f0fdf4" };
    if (type === "reschedule_rejected") return { icon: "🚫", color: "#ef4444", bg: isDark ? "#2d0f0f" : "#fef2f2" };
    return { icon: "🔔", color: "#60a5fa", bg: isDark ? "#0d1b4b" : "#eff6ff" };
  };
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 rounded-2xl"
            ...
            <p className="text-sm mt-1" style={{ color: t.textMuted }}>
              You'll be notified of cancellations and reschedule updates here
            </p>
          </div>
        ) : (
